  ...
  function createSetterProxy(path: string[]): any {
    const handler = {
      ...
      set<T extends ClientMiddleware | ClientMiddleware[]>(
        ...
        const key = prop === "$" ? path.join("/") : [...path, prop].join("/");
          ...
          if (isResultRequested || isContextRequested) {
            ...
            return async (args: any, options: CallOptions = {}) => {
              ...
              for (let i = 1; i <= ctx.path.length; i++) {
                const key = ctx.path.slice(0, i).join("/");
...
async function call(baseUrl: string, ctx: ClientContext) {
  ...
  const url =
    typeof window !== "undefined" && typeof window.document !== "undefined"
      ? new URL(
          `${baseUrl}/${ctx.path.join("/")}`,
          window ? window.location.origin : undefined,
        )
      : new URL(`${baseUrl}/${ctx.path.join("/")}`);
    ...
    xhr.addEventListener("load", () => {
      ...
      resolve(
        new Response(xhr.response, {
          ...
          headers: new Headers({
            "Content-Type":
              xhr.getResponseHeader("Content-Type") || "application/msgpack",
            "X-Execution-Time": xhr.getResponseHeader("X-Execution-Time") || "",
          }),
        }),
      );
    });
